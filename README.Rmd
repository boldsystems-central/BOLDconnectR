...
*These functions are currently internal functions which require external dependencies not included in the package. For their specific usage, please see the details provided below.*
...
Data is downloaded followed by the 'geography' filter applied on it to fetch only the relevant locations (here Ciudad de Mexico,Antioquia Colorado) data.
...
```{r gen.comm.mat2,echo=FALSE,warning=FALSE}
...
#head(grids_output)
#DT::datatable(grids_output,class = 'cell-border stripe')

# View the community matrix
#DT::datatable(comm.data.grid$comm.matrix,class = 'cell-border stripe')

comm.data.grid$comm.matrix

```

## analyze.alphadiv

```{r alphadiv,echo=FALSE,fig.width=6, fig.align='center',warning=FALSE}
...
# DT::datatable(div.res1$richness,class = 'cell-border stripe')
# 
# DT::datatable(div.res1$Shannon_div,class = 'cell-border stripe')
...
```{r betadiv,echo=FALSE,fig.width=8, fig.align='center',warning=FALSE}
...
#DT::datatable(as.matrix(round(beta.div.res2$total.beta,2)),class = 'cell-border stripe')
...
```{r gen.comm.mat,echo=FALSE,fig.width=8, fig.align='center',warning=FALSE}
...
#DT::datatable(geo.viz$geo.df,class = 'cell-border stripe')
head(geo.viz$geo.df)
```
...
```{r align.seq,echo=FALSE,warning=FALSE}
...
#DT::datatable(data.seq.aligned$seq.df,class = 'cell-border stripe')
...
```{r analyze.seq,echo=FALSE,fig.width=6, fig.height=4, fig.align='center',warning=FALSE}
...
#DT::datatable(as.matrix(data.seq.analyze$dist_matrix),class = 'cell-border stripe')
