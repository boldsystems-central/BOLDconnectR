...
#' \emph{Note: } This function is currently an internal function with documentation and can be accessed only by using the `:::` operator  (`BOLDconnectR:::analyze.seq`). Users are required to install and load  the `Biostrings` and `msa` packages before running this function that are maintained by `Bioconductor`.
#'
