  ...
  
{
  ...
  # condition to check if the BIN id is of the correct data type  
  
  if(is.character(bin.id)==FALSE) 
    
  {
    
    warning("BIN fetch.params should be character")
  ...
  bold.df=bold.df%>%
    dplyr::filter(bin_uri %in% !!bin.id)
