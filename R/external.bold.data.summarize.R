...
#'
#' @details
#' `bold.data.summarize` provides summaries for each data type available in the downloaded dataset. The function uses the `skimr::skim()` function to generate a list of data frames. Summaries are generated by one of three `summarize_by` options, a) using `all_data` that considers all data that is entered, b) using `presets`that are defined set of columns representing a specific aspect of the BOLD BCDM data (currently available are: `taxonomy`,`geography`,`sequences`,`attributions`, `ecology_biogeography` and `other_meta_data`) and c) `fields` which lets the user select any specific columns. `na.rm`= TRUE removes all NA values. The summary includes detailed summary statistics (includes counts for NULL values, unique values and the proportion of complete cases), a bar chart showing some of these statistics, especially pertaining to the completeness and uniqueness of the data and a concise summary that has high level dataset profile (number of rows, columns, data type). These results are based on the `summarize_by` argument i.e. summary of only those fields will be generated (unless `all_data` is used). Both `presets` and `fields` by default, are set to NULL. Please note that if the `cols` argument from [bold.fetch()] has been used to filter for specific columns, only those will be summarized by default if `all_data` option is selected. Similarly, `presets` option will not work in cases where the input data by default doesnt have the respective fields. For specific details on the skim output, refers to the `skimr` package documentation.
#'
