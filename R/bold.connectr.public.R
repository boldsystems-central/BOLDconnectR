...
#'
#' @details `bold.connectr.public` downloads publicly available data on BOLD. Data can be retrieved by providing either one or a combination of taxonomy, geography, bins, ids or datasets codes. There is no limit on the data that can be downloaded but complex combinations of the search parameters can lead to web-url character length exceeding the predetermined limit (2048 characters). Single parameter searches are exempt from this limit as downloads are carried out in batches of 5 (values of the parameter). Downloaded data can then be filtered on either one or a combination of arguments (such as institutes, identifiers, altitude, depth etc.). Using the fields argument allows users to select specific columns for inclusion in the final data frame, though, processids and sampleids, are included by default. If the `fields` argument is left as NULL all columns will be downloaded. The names of the columns in the downloaded data are those specified in the the `bold.fields.info`. Note that some values or fields might currently be unavailable but may be accessible future.
#'
