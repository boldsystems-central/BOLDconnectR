...
#' @param site_type A character vector specifying one of two broad categories of `sites` (`locations` or `grids`). Needs to be provided by default.
...
#' @param diversity_profile A character value specifying the type of diversity profile ("richness","preston","shannon","beta","all"). Needs to be provided by default.
...
#' Preston plots are generated using the data from the `prestondistr` results in `ggplot2` featuring cyan bars for observed species (or equivalent taxonomic group) and orange dots for expected counts. The `presence_absence` argument converts the counts (or abundances) to 1s and 0s.
#' Beta diversity values are calculated using `BAT::beta()` function, which partitions the data using the Podani & Schmera (2011)/Carvalho et al. (2012) approach. These results are stored as distance matrices in the output.
#' \emph{Note on the community matrix}: Each cell in this matrix contains the counts (or abundances) of the specimens whose sequences have an assigned BIN, in a given `site_type` (`locations` or `grids`). These counts can be generated at any taxonomic hierarchical level, applicable to one or multiple taxa including `bin_uri`. The `site_type`=`locations` followed by providing a `location_type` can refer to any geographic field (countries, province etc.), and metadata on these fields can be checked using the `bold.fields.info()`. `site_type`=`grids` generates grids based on BIN occurrence data (latitude, longitude) with grid size determined by the user in square meters using the `gridsize` argument. Rows lacking latitude and longitude data (NULL values) are removed when `site_type`=`grids`. Conversely, NULL entries are permitted when `site_type`=`locations`, even if latitude and longitude values are missing. This distinction exists because grids rely on bounding boxes, which require latitude and longitude values. This filtering could impact the richness values and other analyses, as all records for the selected `taxon_rank` that contain `location` information but lack latitude and longitude will be excluded if `site_type`=`grids`. This means that the same dataset could yield different results depending on the chosen `site_type`.
