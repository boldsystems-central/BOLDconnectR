#' fetch data as per the query parameters (processids,sampleids,bin_uri,datasets,projects)
#' @keywords internal
#'
################################## fetch.bold.processid helper function ######################################################

fetch.bold.processid<-function(data.input,
                               param.index,
                               api_key)

{
...

  {
    ...
    json.df=get_data_processid(input_data = data.input,
...
################################## fetch.bold.sampleid helper function ######################################################
...
################################## fetch.bold.bin_uri helper function ######################################################
...

{
  ...
  # base url for fetching the processids using the BIN input
...

  {
    ...
    data_for_bins = data.input[,param.index]

    # get_data_bins is a function to obtain the processids using the BOLD GET API.

    input_data2=get_data_bins(data_for_bins,
                              api.key = api_key)
...

  {
    ...
    batches_bins = generate.batches(data=data.input,
                                    param.index = 1,
                                    batch.size = 99)

    # get_data_bins is used in combination with lapply to generate a list of processids for all batches

    input_data2_list=lapply(batches_bins, function (x) get_data_bins(x,
                                                                     api.key = api_key))
...
################################## fetch.bold.datasets helper function ######################################################
...

{
...

  {
    ...
    data_for_datasets = data.input[,param.index]

    # get_data_bins is a function to obtain the processids using the BOLD GET API.

    input_data2=get_data_datasets(data_for_datasets,
                                  api.key = api_key)
...

  {
    ...
    batches_datasets = generate.batches(data=data.input,
                                        param.index = 1,
                                        batch.size = 99)

    # get_data_bins is used in combination with lapply to generate a list of processids for all batches

    input_data2_list=lapply(batches_datasets, function (x) get_data_datasets(x,
                                                                             api.key = api_key))
    # Convert the list to a data frame of processids
...
################################## fetch.bold.projects helper function ######################################################
